...
export default function ReportsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(await responseDetail(response, copy.calculateFailed));
        return;
      }
      ...
      setError(userFacingError(reason, copy.calculateFailed));
    } finally {
      setLoading(false);
    }
  ...
  async function exportCsv() {
    ...
    try {
      ...
      if (!response.ok) {
        setError(await responseDetail(response, copy.exportFailed));
        return;
      }
      ...
      setError(userFacingError(reason, copy.exportFailed));
    } finally {
      setExporting(false);
    }
  }
      ...
      <section className="space-y-6">
        ...
        {report ? (
          <>
            ...
            <section className="overflow-x-auto" aria-label={copy.ledgerLabel}>
