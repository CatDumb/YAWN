...
export default function PlannerPage() {
  ...
  async function read(response: Response) {
    const body = (await response.json()) as
      PreviewDay[] | { detail?: string; created?: number };
    if (!response.ok) {
      setMessage(
        typeof body === "object" && !Array.isArray(body)
          ? (body.detail ?? copy.saveFailed)
          : copy.saveFailed,
      );
      return null;
    }
  ...
  const loadProjection = useCallback(async () => {
    ...
    if (!response.ok) {
      ...
      setProjectionError(body.detail ?? copy.coverageFailed);
      return;
    }
  ...
  async function saveEdit() {
    ...
    if (!response.ok) {
      const body = (await response.json()) as { detail?: string };
      setMessage(body.detail ?? copy.editChanged);
      return;
    }
    setEditing(null);
  ...
  async function deleteSeries() {
    ...
    if (!response.ok) {
      const body = (await response.json()) as { detail?: string };
      setMessage(body.detail ?? copy.deleteSeriesFailed);
      return;
    }
    setEditing(null);
    setConfirmSeriesDelete(false);
                ...
                <fieldset className="grid gap-3 sm:grid-cols-2">
                  ...
                  <label className="fieldset">
                    ...
                    <select
                      className="select w-full"
                      onChange={(event) =>
                        setEditCommitment(event.target.value)
                      }
