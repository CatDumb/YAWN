...
async function latestOtp(email: string): Promise<string | null> {
  try {
    const messages = await readdir(mailDirectory);
    const candidates = await Promise.all(
      messages.map(async (message) => ({
        contents: await readFile(path.join(mailDirectory, message), "utf8"),
        modified: (await stat(path.join(mailDirectory, message))).mtimeMs,
      })),
    );
    candidates.sort((left, right) => right.modified - left.modified);
    for (const candidate of candidates) {
      if (!candidate.contents.includes(email)) continue;
      const match = candidate.contents.match(/sign-in code is (\d{6})\./);
      if (match && match[1] !== usedOtps.get(email)) return match[1];
    }
  } catch {
...
test("draft, submit, reject, correct, and duplicate WIO lifecycle", async ({
  ...
  await page.getByLabel("Did you work in the office?").selectOption("in_office");
